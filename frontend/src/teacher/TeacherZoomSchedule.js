...
const TeacherZoomSchedule = ({ user }) => {
    ...
    const fetchMeetings = useCallback(async () => {
        try {
            const res = await axios.get(`https://edu-track-c6ml.onrender.com/api/zoom?subject=${user.subject}`);
    ...
    const handleSchedule = async (e) => {
        ...
        const data = {
            ...
            subject: user?.subject || '', teacher: user?.name || 'Teacher', teacherId: user?.id || ''
        };

        try {
            const res = await axios.post('https://edu-track-c6ml.onrender.com/api/zoom', data);
            if (res.data.success) {
    ...
    const deleteMeeting = async (id) => {
        ...
        try {
            const res = await axios.delete(`https://edu-track-c6ml.onrender.com/api/zoom?id=${id}`);
